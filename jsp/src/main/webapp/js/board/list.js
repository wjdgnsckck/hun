			...
			let html = `<tr> <th> 번호 </th> <th> 카테고리 </th>
				...
				pageDto.boardList.forEach( b => {
					html += `<tr> 
								<td> ${ b.bno } </td> 
								<td> ${ b.bcname } </td>
								<td> <a href="/jsp/board/view.jsp?bno=${ b.bno }"> ${ b.btitle } </a> </td> 
								<td> 
									<img src="/jsp/Member/img/${ b.mimg }"/>
									${ b.mid } 
								</td> 
								<td> ${ b.bview } </td>
								<td> ${ b.bdate } </td> 
...
/*
	...
	HTTP URL에 매개변수(파라미터) 전달 ( 쿼리[질의]스트링 방식 )
		...
		- 형태 
			URL?변수명=데이터
			URL?변수명=데이터&변수명=데이터
			http://localhost:80/jsp/board/view.jsp?bno=3
			href="/jsp/board/view.jsp?bno=${ b.bno }
