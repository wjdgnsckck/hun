...
public class JDBC연동 {
		public static void main(String[] args) {
			// ---------------------------------예외가 발생할것 같은 코드들을 try{} 안에---------------------//
			try {	//JDBC 드라이버 클래스들의 예외처리
			//1. p.908 : 현재 프로젝트에 JDBC 파일을 build path add 하기
			//2. p.909 : DB연결하기 위해 JDBC Driver 클래스를 찾아서 메모리 로딩
					//Class.forName("oracle.jdbc.OracleDriver");
			Class.forName("com.mysql.cj.jdbc.Driver");	//mySQL실행
		 ...
		 예외처리 : 
		 	...
		 	-예외[오류] 발생했을때 흐름 제어 [ *만약에 오류 발생했을때 다른 코드로 흐름 제어 ]
		  	-try { } catch { }
		  		...
		  		* try{ } 안에서 예외가 발생하는 순간 바로 catch{ } 이동하기 때문에 예외발생한 코드 아래로는 실행안됨
		  	-예외[오류]가 발생할것 같은코드
