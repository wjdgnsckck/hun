...
/*
 1.jar파일을 이클립스에 넣는다	
 2.jar파일 우클릭 Build Path 클릭한다
 3.Build Path 에 add to Build Path 클릭한다.
		...
		- 테이블 용어   /  테이블 [ 표 ]
            ...
            -데이터 필드 타입 [Mysql기준 *DBMS 조금씩 다름]
				...
				2.실수
					float(4) , double(8)
				3.문자
					char(문자길이) 	  : 고정길이  	char(3) ->'ab' -> 3바이트	[*저장할 데이터의 글자수가 제한되는곳]
                    varchar(문자길이) : 가변길이		varchar(3)->'ab'->2바이트	[*저장할 데이터의 글자수가 제한이없는곳]
				4.날짜
					 date(날짜) 	  : YYYY-MM-DD 
					time(시간)      	:HH:MM:SS
...
#3. DB 사용 선택
use sqldb1web;
#4. 만약에 생성할 테이블의 이름이 존재하면 삭제..
drop table if exists member;
#5.테이블 생성 (필드명 타입 , 필드명타입)
...
/*
	문제2:
		1.웹개발 하는데 DB저장소'sqldb1web2'이름으로 DB생성
        2.해당 db에 board 테이블 생성
        no 게시물번호(타입 : 최대21억정도)
		title 게시물제목(문자열 최대20)
        content 게시물내용(문자열 최대 6만5천이상)
        view(최대 21억정도)
        datetime(날짜/시간)
*/
