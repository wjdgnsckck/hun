...
function 추가함수(){      // f start
	...
	학생리스트.indexOf(name) != -1 ? alert('로그인') : 학생리스트.push( name )
...
function 삭제함수(){		//f start
	...
	let name=snameInput.value
	//* 삭제할려면 인덱스 필요 인덱스 몰라 --->인덱스 찾기
	//3. 삭제할 데이터의 인덱스(저장된 순번)
	let index =학생리스트.indexOf(name)
		// 해당 데이터가 배열에 존재하면 0~찾은번호 없으면-1
	//4. 배열내 인덱스를 이용한 데이터 삭제
	//* 유효성검사 : 데이터가 개발자가 원하는 데이터인지 검사하는 과정
	index != -1 ? 학생리스트.splice(index , 1) : alert('없다.');
